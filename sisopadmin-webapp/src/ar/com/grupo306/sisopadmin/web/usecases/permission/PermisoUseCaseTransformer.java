...
public class PermisoUseCaseTransformer implements UseCaseTransformer {
	...
	public Permiso createPermiso(Class useCaseModelClass, Class<? extends UseCaseModelMode> modeClass, Method method) throws InstantiationException, IllegalAccessException {
		...
		this.addDescription(useCaseModelClass, modeClass, permiso, method);
	...
	private void addDescription(Class useCaseModelClass, Permiso permiso, Method method) {
		this.addDescription(useCaseModelClass, null, permiso, method);
	}
	
	private void addDescription(Class useCaseModelClass, Class<? extends UseCaseModelMode> modeClass, Permiso permiso, Method method) {
		Description descAnnotation = method.getAnnotation(Description.class);
		if(descAnnotation != null) {
			permiso.setDescription(ClassUtils.getShortClassName(useCaseModelClass).replaceFirst("UseCaseModel", "") + " - " + descAnnotation.value() + (modeClass != null ? " (" + ClassUtils.getShortClassName(modeClass) + ")" : ""));
		}
		else {
