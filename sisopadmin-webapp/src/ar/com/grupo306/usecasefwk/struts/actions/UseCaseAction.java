...
public abstract class UseCaseAction extends ModularizedDispatchAction  implements UseCase {
	...
	protected ActionForward invokeDispatchMethod(Method method, ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		UseCaseContext useCaseContext = new UseCaseContext(mapping, request, response);
		...
		try {
			...
			// Si el response ya fue envíado al cliente entonces se dirige al forward null para que no
			// genere una excepción por escritura múltiple en el response.
			return response.isCommitted() ? null : useCaseContext.getActionForward();
		}
		...
		catch(SisopAdminSecurityException excep) {
	...
	protected UseCaseContext resolveAndInvokeMethod(UseCaseContext useCaseContext, Method method, ActionForm form, HttpServletRequest request) throws Exception {
		try {
			...
			if("initUseCase".equals(method.getName())) {
				useCaseModel = this.createUseCaseModel(useCaseContext, method, request);
				// Se agrega el model con el proxy de persistencia para que sea éste el que se apile.
				useCaseContext.addElement(UseCaseFWKConstants.USECASEMODEL_PROXIED_KEY, useCaseModel);
...
//				}
			...
			logger.debug("Invocando el método '" + method.getName() + "' sobre el UseCaseModel '" + useCaseModel + "'...");
	 ...
	 */
	private UseCaseModel createUseCaseModel(UseCaseContext context, Method method, HttpServletRequest request) {
		try {
			...
			// Se obtiene el parámetro de manera artesanal.
			if(stack.hasUseCaseParameters()) {
				// En caso de haber indicado explícitamente el modo, se lo setea.
				UseCaseModelMode mode = (UseCaseModelMode) ((UseCaseParametersElement) stack.viewStackElement()).getParameters().get(UseCaseFWKConstants.USECASEMODELMODE_KEY);
				if(mode != null) {
