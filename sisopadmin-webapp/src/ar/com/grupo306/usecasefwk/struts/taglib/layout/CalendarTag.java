...
public class CalendarTag extends TagSupport {
	...
	private static String DEFAULT_STYLE = "formLabel";
	...
	private String resolveStyle() {
		return this.style != null ? this.style : DEFAULT_STYLE; 
	}
	...
	private void createInputDescriptionHTML(StringBuffer buffer, int fieldMode) {
		buffer.append("\n<tr><th valign='top' class='formLabel'>" + this.resolveDescription() + "</th>");		
	}
	...
	private void createInputDateHTML(StringBuffer buffer, int fieldMode) {
		buffer.append("\n<td valign='top'>");
		if(fieldMode != AbstractModeFieldTag.MODE_INSPECT && fieldMode != AbstractModeFieldTag.MODE_READONLY) {
			buffer.append("\n<input id=\"" + this.resolveId() + "\" name=\"" + this.resolveName() + "\" size=\"11\" readonly='readonly' " +
					"type=\"text\" title=\"" + this.resolveToolTip() + "\"  value=\""
					+ this.resolveInitialDate() + "\" class='" + this.resolveStyle() + "'>&nbsp;");
		}
		else {
			buffer.append("\n<span id='" + this.resolveId() + "' class='" + this.resolveStyle() + "'>" + this.resolveInitialDate() + "</span>");
		}
	}
	...
	private void createCalendarHTML(StringBuffer buffer, int fieldMode) {
		if(fieldMode != AbstractModeFieldTag.MODE_DISABLED && fieldMode != AbstractModeFieldTag.MODE_INSPECT && fieldMode != AbstractModeFieldTag.MODE_READONLY) {
			buffer.append("<a href=\"javascript:show_calendar('" + this.getId() + "');\" " +
				"onmouseover=\"window.status='Calendario';return true;\" onmouseout=\"window.status='';" +
				"return true;\">");
			buffer.append("<img valign='bottom' src=\"" + this.resolveImage() +"\" border=\"0\"  title=\" " + 
					this.resolveToolTip() +"\">");
